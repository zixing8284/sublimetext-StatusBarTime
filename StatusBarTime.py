...
class StatusBarTime(sublime_plugin.EventListener):
    ...
    def on_activated(self, view):
        ...
        clock_Type = settings.get(STATUSBARTIME_CLOCK_TYPE_KEY)
        # Get setting for only in view display
        onlyinview = settings.get(
            STATUSBARTIME_CLOCKDISPLAY_ONLYINVIEW_KEY, True)
        ...
        if not clock_Type:
            Timer(settings.get(STATUSBARTIME_FORMAT_KEY, DEFAULT_FORMAT)
                  ).displayTime(view, update_interval, onlyinview, lefty)
        else:
...
class Timer():
    ...
    def displayTime(self, view, delay, onlyinview, lefty):
        if lefty:
            # just add "__" in front, it stticks left
        ...
        if actwin:
            if not onlyinview or (actwin.active_view() and actwin.active_view().id() == view.id()):
                sublime.set_timeout(lambda: self.displayTime(
                    view, delay, onlyinview, lefty), delay)
        else:
            view.set_status(self.status_key, '')
        return

    # Method for handling uptime display
    def displayUpTime(self, view, startTime, delay, onlyinview, lefty):
        ...
        if days:
            out += str("%s days, " % (days))
        if hours:
            out += str("%02d:%02d:%02d" % (hours, minutes, seconds))
        elif minutes:
            out += str("%02d:%02d" % (minutes, seconds))
        elif seconds:
        ...
        if actwin:
            if not onlyinview or (actwin.active_view() and actwin.active_view().id() == view.id()):
                sublime.set_timeout(lambda: self.displayUpTime(
                    view, startTime, delay, onlyinview, lefty), delay)
        else:
            view.set_status(self.status_key, '')
        return
